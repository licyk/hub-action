...
function Write-Log {
    ...
    param(
        ...
        [string]$Level = "INFO",
        [string]$Name = "InvokeAI Installer"
    )
...
function Get-Version {
    ...
    $minor = $ver[-2]
    $micro = $ver[-1]
    Write-Log "InvokeAI Installer 版本: v${major}.${minor}.${micro}"
}
...
function Update-Installer {
    ...
    if (`$script:BuildMode) {
        Write-Log `"InvokeAI Installer 构建模式已启用, 跳过 InvokeAI Installer 更新检查`"
        return
    }
    ...
    ForEach (`$url in `$urls) {
        Write-Log `"检查 InvokeAI Installer 更新中`"
        try {
            `$web_request_params = @{
                Uri = `$url
                UseBasicParsing = `$true
            ...
            `$i += 1
            if (`$i -lt `$urls.Length) {
                Write-Log `"重试检查 InvokeAI Installer 更新中`" -Level WARNING
            } else {
                Write-Log `"检查 InvokeAI Installer 更新失败`"
                return
            }
        }
    }

    if (`$latest_version -le `$script:INVOKEAI_INSTALLER_VERSION) {
        Write-Log `"InvokeAI Installer 已是最新版本`"
        return
    }
...
function Add-Shortcut {
    ...
    `$IconConfig = @{
        `"windows`" = @{
            FileName = `"invokeai_icon.ico`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/invokeai_icon.ico`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/invokeai_icon.ico`")
        }
        `"linux`"   = @{
            FileName = `"invokeai_icon.png`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/invokeai_icon.png`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/invokeai_icon.png`")
        }
        `"macos`"   = @{
            FileName = `"invokeai_icon.icns`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/invokeai_icon.icns`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/invokeai_icon.icns`")
        }
    }
    if ((!(Test-Path (Join-NormalizedPath `$PSScriptRoot `"enable_shortcut.txt`"))) -and (!(`$script:EnableShortcut))) {
        return
    }
    Write-Log `"检测到 enable_shortcut.txt 配置文件 / -EnableShortcut 命令行参数, 开始检查 InvokeAI 快捷启动方式中`"
...
function Test-WebUIEnv {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_check_env.txt`")) -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 InvokeAI 运行环境检测, 这可能会导致 InvokeAI 运行环境中存在的问题无法被发现并解决`" -Level WARNING
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        if (`$req) {
            ...
            Write-Log `"InvokeAI 出现异常, 已退出`" -Level ERROR
        }
        Read-Host | Out-Null
    }
}
...
".Trim()
    ...
    Write-Log "$(if (Test-Path (Join-NormalizedPath $script:InstallPath "update_node.ps1")) { "更新" } else { "生成" }) update_node.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path (Join-NormalizedPath $script:InstallPath "update_node.ps1") -Value $content
}
...
function Download-Installer {
    ...
    ForEach (`$url in `$urls) {
        Write-Log `"正在下载最新的 InvokeAI Installer 脚本`"
        `$web_request_params = @{
            Uri = `$url
            UseBasicParsing = `$true
        ...
        Invoke-WebRequest @web_request_params
        if (`$?) {
            Write-Log `"下载 InvokeAI Installer 脚本成功`"
            break
        } else {
            Write-Log `"下载 InvokeAI Installer 脚本失败`" -Level ERROR
            `$i += 1
            if (`$i -lt `$urls.Length) {
                Write-Log `"重试下载 InvokeAI Installer 脚本`" -Level WARNING
            } else {
                Write-Log `"下载 InvokeAI Installer 脚本失败, 可尝试重新运行 InvokeAI Installer 下载脚本`" -Level ERROR
                return `$false
            }
        }
    }
...
function Main {
    ...
    if (`$status) {
        Write-Log `"运行 InvokeAI Installer 中`"
        `$arg = Get-LocalSetting
        `$extra_args = Get-ExtraArgs
        `$script_path = Join-NormalizedPath `$PSScriptRoot `"cache`" `"invokeai_installer.ps1`"
        try {
...
".Trim()
    ...
    Write-Log "$(if (Test-Path (Join-NormalizedPath $script:InstallPath "launch_invokeai_installer.ps1")) { "更新" } else { "生成" }) launch_invokeai_installer.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path (Join-NormalizedPath $script:InstallPath "launch_invokeai_installer.ps1") -Value $content
}
...
function Test-EnvIntegrity {
    `$items = @(
        ...
        @{ n=`"InvokeAI`"; p=(Join-NormalizedPath `$env:CORE_PREFIX `"main.py`"); t=`"file`" },
        @{ n=`"uv`"; m=`"uv`" },
        @{ n=`"PyTorch`"; m=`"torch`" },
...
function Main {
    ...
    while (`$true) {
        ...
        switch (`$choice) {
            ...
            `"13`" { Start-Process `"https://github.com/licyk/sd-webui-all-in-one/blob/main/docs/invokeai_installer.md`" }
...
function Copy-InstallerConfig {
    Write-Log "为 InvokeAI Installer 管理脚本复制 InvokeAI Installer 配置文件中"
...
function Use-InstallMode {
    ...
    if ($script:BuildMode) {
        Use-BuildMode
        Write-Log "InvokeAI 环境构建完成, 路径: $script:InstallPath"
    } else {
...
function Use-BuildMode {
    ...
    if ($script:BuildWithUpdateNode) {
        ...
        if ($script:UseCustomGithubMirror) { $launch_args.Add("-UseCustomGithubMirror", $script:UseCustomGithubMirror) }
        if ($script:CorePrefix) { $launch_args.Add("-CorePrefix", $script:CorePrefix) }
        Write-Log "执行 InvokeAI 扩展更新脚本中"
        . (Join-NormalizedPath $InstallPath "update_node.ps1") @launch_args
    }
...
function Main {
    ...
    if ($script:UseUpdateMode) {
        ...
        if ($script:BuildMode) { Write-Log "InvokeAI Installer 构建模式已启用" }
